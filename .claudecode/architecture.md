...
* **Animação (perseguição da ponta):** o scroll não alimenta o `pathLength` diretamente — comprimento não é proporcional à altura (varreduras horizontais consomem comprimento sem descer). A ponta persegue uma **âncora a ~45% do viewport** (deslizando até a base no fim da página, para o desenho terminar no rodapé): um mapa amostrado **y → fração do comprimento** (bem definido pela monotonia em y) converte a âncora na fração-alvo, e a fração desenhada segue o alvo em `useAnimationFrame` com uma **mola criticamente amortecida em forma fechada** (estável para qualquer dt, inclusive frames de jank; sem oscilação nem overshoot — a linha nunca "desdesenha" sozinha) e **teto de velocidade de desenho** que transforma saltos grandes do alvo (cauda inicial quase horizontal, pulo ao rodapé) em varredura contínua. A **rigidez muda com o ponteiro primário** (`pointer: coarse/fine`, mesmo idioma `useSyncExternalStore` do reduced motion), porque o scroll é outro: toque = contínuo com inércia → mola rígida (ω=20rad/s, teto 6000px/s), a ponta desce **junto com a tela** mesmo em flings de ~2800px/s; mouse = degraus discretos → mola macia (ω=10rad/s, teto 3000px/s), cada tick de roda vira uma **caminhada A→B de ~0,7s partindo de velocidade zero**. O estado da ponta é a **fração** (+ velocidade da mola), não o y — nos trechos quase horizontais y mal distingue posições ao longo da curva; a ponta nasce presa no início do path, então a página carrega sem traço visível. Alternativas medidas e descartadas (instrumentação Playwright, 3 cenários: ticks isolados, contínuo ~800px/s, flings ~2800px/s): perseguição proporcional ≤120px/s em y (ponta a 1326px acima do viewport — cada tick lia-se como teleporte da linha inteira), alisamento exponencial único (front-loaded: dardo por tick no mouse, tartaruga em fling no toque) e spring rígido convergido da V2.1 ("arranques" por tick). A fração alimenta `pathLength` em dois `motion.path` — camada de glow (traço largo, `blur` + keyframe `scroll-line-pulse` de opacidade em `globals.css`) e linha principal fina por cima. Um fade via `useTransform` (opacity 0→1 no primeiro 1% do draw) esconde o ponto que o `strokeLinecap` redondo deixaria no início do path em repouso — no glow, o fade vive num `<g>` externo para não disputar a `opacity` com a animação CSS do pulse.
...
| `tests/scroll-line.spec.ts` | Scroll Progress Line: overlay decorativo (`aria-hidden`, `pointer-events: none`, `position: absolute`) e draw progressivo do path com o scroll (`stroke-dasharray` >0,5 no fim da página, poll de 60s — o draw ao rodapé converge em ~5s sob o teto de velocidade de desenho, §5.5) | 1 |
