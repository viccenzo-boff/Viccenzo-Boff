...
Título `h2` sobre `bg-muted` **não pode** usar `text-muted-foreground` (4,39:1 em light — abaixo de 4,5:1); usa `text-foreground/70` (7,38:1 light / 7,73:1 dark). Vale para as 4 seções `bg-muted` (`metrics-dashboard`, `projects`, `skills-matrix`, `academic-background` — esta também na linha de status da graduação e nos níveis de idioma). Nas 5 seções `bg-background`, o `h2` usa `text-muted-foreground` normalmente. Badges usam sempre `bg-background` + `text-muted-foreground`, independentemente do fundo — garante ≥4,5:1 nos dois temas.
