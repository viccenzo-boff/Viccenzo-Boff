...
**V2.2 commitada pelo usuário** (`1479d07` — linha reancorada ao DOM, não cruza títulos em nenhum viewport). Na revisão visual da V2.2 (2026-07-11, capturas do usuário) surgiu a **V2.3** (pontas não perfeitamente redondas em alguns portões — plano e relatório em §8) e, na sequência, a **V2.4** (linha "teletransportava" durante o scroll — perseguição reescrita: mola criticamente amortecida com rigidez por tipo de ponteiro + teto de velocidade de desenho). Ambas no working tree aguardando commit. O usuário segue reportando falta de fluidez; o post-mortem técnico completo da linha (todas as abordagens, causas de falha, hipóteses em aberto e recomendações) está em `docs/scroll-line-postmortem.md`.
...
### Pendências da V2.5 — Vínculo 1:1 da Scroll Progress Line (2026-07-12)

**Só o usuário pode resolver:**

* **Validar o feel do vínculo 1:1 e commitar o working tree** (que hoje soma V2.3 — pontas G2 — e V2.5 — draw 1:1 sem física; a V2.4 foi substituída antes do commit). Testar com roda de mouse no desktop e com o dedo no celular real; antes de concluir qualquer coisa, conferir no DevTools que o container `[data-testid="scroll-progress-line"]` tem `data-line-version="v2.5-1to1"` (produção ainda roda a V2.2 — a pior versão medida). Mensagem sugerida: `fix(ui): bind scroll line 1:1 to scroll position, removing chase physics`.
...
**Backlog opcional (evoluções, só se o feel 1:1 for aprovado):**
...
**Decisão tomada que aguarda confirmação:**
...
**Novas tarefas descobertas durante a execução (a triar):**
